...
class SecurityValidator(ast.NodeVisitor):
    ...
    def visit_Constant(self, node: ast.Constant) -> None:
        """Detect format string that accesses blocked attributes via .format() or f-strings."""
    ...
    def visit_JoinedStr(self, node: ast.JoinedStr) -> None:
        """Detect f-string that accesses blocked attributes."""

        for value in node.values:
            if isinstance(value, ast.FormattedValue) and value.value:
